...
$(document).on('change', '.InvoiceContent', function () {
    InvoiceActionManagement($(this));
});

function InvoiceActionManagement(htmlcontrol) {
...
function GetClientShippingAddress() {
    ...
    if (Client_ID != "") {
        ...
        $("#ClientList").focus();
        $("#ToInvoice .select2-selection").css({ "border-color": "#ff4081" });
    }
}
...
$(document).on('change', '#ClientList', function () {
    ...
    if (Client_ID != "") {
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#InvoiceToAddressText").val(response[0]);
        ...
        $("#ClientList").focus();
        $("#ToInvoice .select2-selection").css({ "border-color": "#ff4081" });
        $("#InvoiceToAddressDiv input,#InvoiceToAddressDiv textarea").val('');   
    }
});

function GetClientBillingAddress() {
...
$(document).on('change', '#ItemList', function () {
    ...
    if (Item_Id != "") {
        ...
        $("#invoiceitemvalidate").addClass("hide");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $(".invoice-item #Rate").val(response[0].Sale_Price);
...
$(document).on("click", "#AddNewItem-row", function () {
    ...
    if ($(".invoice-item #ItemList").val() != "" && $(".invoice-item #Qty").val() != "" && $(".invoice-item #Rate").val() != "") {
        ...
        $("#invoiceitemvalidate").removeClass("hide");
    }
});
...
$(document).on('click', '#filter-invoice', function () {

    $("#hid-page-no").val($(this).attr("data-value"));
    Getinvoicedatatable();
});

function Getinvoicedatatable() {

    var valdata = $("#FormFilter").serialize();

    $.ajax({
        url: "/Invoice/InvoiceDataTable",
        type: "Post",
        data: valdata,
        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
        beforeSend: function () {
...
$("#ShortClientSearch").on("input", function () {
    $("#ClientFilter").val($(this).val());
    GetShortinvoicedatatable();
});

$(document).on('click', '#Shortfilter-invoice', function () {

    $("#hid-page-no").val($(this).attr("data-value"));
    GetShortinvoicedatatable();
});

function GetShortinvoicedatatable() {
    var valdata = $("#FormFilter").serialize();

    $.ajax({
        url: "/Invoice/ShortDataTable",
        type: "Post",
        data: valdata,
        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
        beforeSend: function () {
...
$(document).on('click', '#view-invoice', function () {
    ...
    if ($("#invoice-id").val() != id) {
        $("#invoice-id").val(id);
        $("#loading").removeClass("hide");
        $("#preview-iframe").attr("src", "../Document/Print.aspx?Invoiceid=" + id + "");
...
$(document).on('click', '#invoice-action-status', function () {

    var id = $("#invoice-id").val();

    $.ajax({
        url: "/Invoice/InvoiceStatusUpdate",
        type: "Get",
...
function GetActionButtons() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            action_buttons += '<a class="waves-effect waves-light btn mb-1 mr-1 pl-1" href="../Document/Print.aspx?DocumentType=pdf&Invoiceid=' + id + '" target="_blank"><i class="material-icons left">picture_as_pdf</i> PDF</a>';
          
            action_buttons += '<a class="waves-effect waves-light btn-light-indigo btn mb-1 mr-1 pl-1" href="../Invoice/AddInvoice?Invoiceid=' + id + '"><i class="material-icons left">create</i> Edit</a>';

            action_buttons += '<button class="waves-effect waves-light red btn mb-1 mr-1 pl-1" id="invoice-email" value="Email"><i class="material-icons left">email</i> Email</button>';
            action_buttons += '<button class="waves-effect waves-light green btn mb-1 mr-1 pl-1" id="po-action-status" value="Payment"><i class="material-icons left">payment</i> Payment</button>';

            if (response.Status == "Cancelled") {
                action_buttons += '<button class="waves-effect waves-light orange btn mb-1 mr-1 pl-1 disabled" id="invoice-action-status" value="Cancelled"><i class="material-icons left">cancel</i> Cancelled</button>';
            }
            else {
                action_buttons += '<button class="waves-effect waves-light orange btn mb-1 mr-1 pl-1" id="invoice-action-status" value="Cancelled"><i class="material-icons left">cancel</i> Cancel</button>';
            }

            if (response.Status == "Cancelled") {
                action_buttons += '<button class="btn indigo waves-effect waves-light mb-1 mr-1 pl-1" id="invoice-action-status" value="Open"><i class="material-icons left">lock_open</i>Reopen</button>';
            }
